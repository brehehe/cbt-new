    ...
    <div class="bg-white rounded-2xl shadow-2xl w-full transform transition-all scale-95 duration-300 ease-out animate-fade-in"
        ...
        <div class="flex justify-end gap-2 px-6 py-4 border-t">
            ...
            <button wire:click='submit'
                class="px-4 py-2 {{ in_array(config('app.name_slug'), ['ups_tegal', 'unimma','unidayan']) ? 'bg-[#2b7fff]' : 'bg-[#f58634]' }}pp.name{{ in_array(config('app.name_slug'), ['ups_tegal', 'unimma','unidayan']) ? 'bg-[#2b7fff]' : 'bg-[#f58634]' }}'ups_tegal' ? 'bg-[#2b7fff]' : 'bg-[#f58634]' }} hover:{{ in_array(config('app.name_slug'), ['ups_tegal', 'unimma','unidayan']) ? 'bg-[#2b7fff]' : 'bg-[#f58634]' }} text-white rounded-lg shadow transition">
                Simpan
            </button>
        </div>
    </div>
