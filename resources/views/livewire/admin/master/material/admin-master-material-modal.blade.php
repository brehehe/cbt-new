    ...
    <div class="bg-white rounded-2xl shadow-2xl w-full transform transition-all scale-95 duration-300 ease-out animate-fade-in" style="max-width: 100vh">
        ...
        <div class="px-6 py-4 text-gray-600">
            ...
            <div class="mb-4">
                ...
                <input type="text" id="name" wire:model.defer="name" placeholder="Nama Materi" class="mt-1 form-control">
                @error('name')
                    <p class="text-sm text-red-600 mt-1">{{ $message }}</p>
                @enderror
            </div>
            <div class="mb-4">
                <label for="level" class="block text-sm font-medium text-gray-700">Level Materi <span class="text-red-600">*</span></label>
                <input type="text" id="level" wire:model.defer="level" placeholder="Level Materi" class="mt-1 form-control">
                @error('level')
                    <p class="text-sm text-red-600 mt-1">{{ $message }}</p>
                @enderror
            </div>
