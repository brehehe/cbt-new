<div>
    ...
    <div class="mb-4">
        <div class="flex items-center justify-between">
            <div>
                <h1 class="text-2xl font-bold text-[#f58634]">Regulasi</h1>
            </div>
            <div>
            ...
            <table class="table">
                ...
                <tbody>
                    @forelse ($regulations as $index => $regulation)
                        <tr>
                            <td class="center">{{ $regulations->firstItem() + $index }}</td>
                            <td>{{ $regulation->description ?? '-' }}</td>
                            <td>{{ $regulation->type == 'licensing' ? 'Wajib' : 'Larangan' }}</td>
                            {{-- <td>{{ $regulation->phone ?? '-' }}</td> --}}
                            <td class="center">
                                <div class="flex items-center">
                                    <!-- Tombol Edit -->
                                    <button
        ...
        <div class="px-5 py-4 bg-gray-50/80 border-t border-gray-200">
            <div class="flex items-center justify-between">
                <div class="text-sm text-gray-700">
                    Menampilkan <span class="font-medium">{{ $regulations->firstItem() }}</span> sampai <span
                        class="font-medium">{{ $regulations->lastItem() }}</span> dari <span
                        class="font-medium">{{ $regulations->total() }}</span> hasil
                </div>
                <div>
                    <nav class="relative z-0 inline-flex rounded-md shadow-sm -space-x-px" aria-label="Pagination">
                        {{ $regulations->links('vendor.livewire.custom') }} <!-- Menampilkan pagination -->
                    </nav>
                </div>
            </div>
        </div>
