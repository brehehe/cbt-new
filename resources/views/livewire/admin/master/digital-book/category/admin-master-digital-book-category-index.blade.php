@section('title', 'Kategori Buku Digital')
<div>
    @include('livewire.admin.master.digital-book.category.admin-master-digital-book-category-modal')
    <div class="mb-4">
        <div class="flex items-center justify-between">
            <div>
                <h1 class="text-2xl font-bold text-[color:var(--primary)]">
                    Kategori Buku Digital</h1>
                {{-- <p class="text-gray-600">Kelola kategori pengelompokan buku digital dan bahan ajar.</p> --}}
            </div>
            <div>
            ...
            <table class="table">
                <thead>
                    <tr>
                        ...
                        <th>Nama Kategori</th>
                        <th>Deskripsi</th>
                        <th class="center">Jumlah Buku</th>
                ...
                <tbody>
                    @forelse ($categories as $index => $category)
                        <tr>
                            <td class="center">{{ $categories->firstItem() + $index }}</td>
                            <td>{{ $category->name }}</td>
                            <td>{{ $category->description ?? '-' }}</td>
                            <td class="center">
                                <span class="badge bg-indigo-100 text-indigo-800 rounded-full px-2.5 py-0.5 text-xs font-semibold">
        ...
        <div class="px-5 py-4 bg-gray-50/80 border-t border-gray-200">
            <div class="flex items-center justify-between">
                <div class="text-sm text-gray-700">
                    Menampilkan <span class="font-medium">{{ $categories->firstItem() }}</span> sampai <span
                        class="font-medium">{{ $categories->lastItem() }}</span> dari <span
                        class="font-medium">{{ $categories->total() }}</span> hasil
                </div>
                <div>
