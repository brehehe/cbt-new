    ...
    <div class="flex min-h-full justify-center p-4 items-start w-full">
        ...
        <div class="flex justify-between items-center p-6 border-b">
            <div class="flex items-center gap-2">
                ...
                <h2 class="text-xl font-semibold text-gray-800">{{ $data_id ? 'Edit Data ' . lecturer_label() : 'Tambah Data ' . lecturer_label() }}
                </h2>
            </div>
            ...
            <div class="grid grid-cols-2 gap-4 mb-4">
                ...
                <div class="mb-4">
                    ...
                    <input id="name" type="text" wire:model.defer="name"
                        placeholder="Contoh : {{ is_shorinji() ? 'Ahmad Suharto' : 'Dr. Ahmad Suharto' }}"
                ...
                <div class="mb-4">
                    ...
                    <input id="email" type="email" wire:model.defer="email"
                        placeholder="Contoh : {{ is_shorinji() ? 'pelatih@perkemi.org' : 'dosen@university.ac.id' }}" class="mt-1 form-control">
                    @error('email')
                        <p class="text-sm text-red-600 mt-1">{{ $message }}</p>
                    @enderror
                </div>
                ...
                {{-- ID Pelatih / Dosen --}}
                <div class="mb-4">
                    <label for="lecturer_id" class="block text-sm font-medium text-gray-700">ID {{ lecturer_label() }}
                        @if(!is_shorinji()) <span class="text-red-600">*</span> @endif
                    </label>
                    <input id="lecturer_id" type="text" wire:model.defer="lecturer_id"
                        placeholder="Contoh : {{ is_shorinji() ? 'PLT001' : 'LEC001' }}"
                        class="mt-1 form-control">
                    @error('lecturer_id')
                        <p class="text-sm text-red-600 mt-1">{{ $message }}</p>
                    @enderror
                ...
                @if(!is_shorinji())
                    ...
                    {{-- Jabatan Fungsional (khusus kampus) --}}
                    <div class="mb-4">
                    ...
                    {{-- Tingkat Pendidikan (khusus kampus) --}}
                    <div class="mb-4">
                    ...
                    {{-- Spesialisasi (khusus kampus) --}}
                    <div class="mb-4">
                        <label for="lecturer_specialization" class="block text-sm font-medium text-gray-700">Spesialisasi</label>
                    ...
                    {{-- Minat Penelitian (khusus kampus) --}}
                    <div class="mb-4">
                ...
                <div class="mb-4">
                    <label for="lecturer_status" class="block text-sm font-medium text-gray-700">Status {{ lecturer_label() }}</label>
                    <select id="lecturer_status" wire:model.defer="lecturer_status" class="mt-1 form-control">
                        <option value="active">Aktif</option>
                        <option value="inactive">Non-Aktif</option>
                        <option value="retired">{{ is_shorinji() ? 'Pensiun / Tidak Aktif' : 'Pensiun' }}</option>
                    </select>
                ...
                {{-- Tipe (selalu tampil) --}}
                <div class="mb-4">
                    <label for="lecturer_type" class="block text-sm font-medium text-gray-700">Tipe {{ lecturer_label() }}</label>
                ...
                <div class="mb-4">
                    <label for="gender" class="block text-sm font-medium text-gray-700">Jenis Kelamin
                        @if(!is_shorinji()) <span class="text-red-600">*</span> @endif
                    </label>
                ...
                {{-- Tempat Lahir --}}
                <div class="mb-4">
                    <label for="birth_place" class="block text-sm font-medium text-gray-700">Tempat Lahir
                        @if(!is_shorinji()) <span class="text-red-600">*</span> @endif
                    </label>
                ...
                {{-- Tanggal Lahir --}}
                <div class="mb-4">
                    <label for="birth_date" class="block text-sm font-medium text-gray-700">Tanggal Lahir
                        @if(!is_shorinji()) <span class="text-red-600">*</span> @endif
                    </label>
                ...
                {{-- Kota --}}
                <div class="mb-4">
                    <label for="city" class="block text-sm font-medium text-gray-700">Kota
                        @if(!is_shorinji()) <span class="text-red-600">*</span> @endif
                    </label>
                    ...
                    <div class="md:col-span-2 mb-4">
                        ...
                        <div x-data="{ show: false }" class="relative">
                            <input :type="show ? 'text' : 'password'" id="password" wire:model.defer="password"
                                placeholder="{{ is_shorinji() ? 'Opsional, default: password123' : 'Minimal 8 karakter' }}"
                        ...
                        @enderror
                        @if(is_shorinji())
                            <p class="text-xs text-gray-400 mt-1">Jika dikosongkan, password default adalah <strong>password123</strong></p>
                        @endif
                    </div>
                @endif
            </div>
