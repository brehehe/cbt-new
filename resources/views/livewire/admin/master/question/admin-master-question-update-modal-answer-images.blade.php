<div wire:ignore.self id="modal-answer-images"
    ...
    <div class="bg-white rounded-2xl shadow-2xl w-full transform transition-all scale-95 duration-300 ease-out animate-fade-in"
        ...
        <div class="flex justify-between items-center p-6 border-b">
            <div class="flex items-center gap-2">
                ...
                <h2 class="text-xl font-semibold text-gray-800">Data Gambar Jawaban <b>{{ $answer_alphabet }}</b></h2>
            </div>
        ...
        <div class="px-6 py-4 text-gray-600 overflow-auto" style="max-height: 85vh"
            wire:key='answer-images-{{ $answer_id }}'>
            <div class="mb-4">
                <label for="answer_images" class="block text-sm font-medium text-gray-700 mb-2">Pilih & Kelola Gambar Jawaban</label>
                
                <!-- Grid Preview Gambar -->
                <div class="grid grid-cols-2 md:grid-cols-4 gap-4 mb-4">
                    @foreach ($answer_images as $index => $img)
                        <div class="relative group border rounded-lg overflow-hidden flex items-center justify-center bg-gray-50 h-32">
                            ...
                            @endif
                            <button type="button" wire:click="removeAnswerImage({{ $index }})" class="absolute top-1 right-1 bg-red-500 text-white rounded-full w-6 h-6 flex items-center justify-center opacity-70 group-hover:opacity-100 transition shadow">
                    ...
                    <label class="border-2 border-dashed border-gray-300 rounded-lg flex flex-col items-center justify-center h-32 hover:bg-gray-50 hover:border-gray-400 transition cursor-pointer text-gray-500 group">
                        ...
                        <input type="file" wire:model="answer_new_images" multiple accept=".jpg,.jpeg,.png,.webp,.mp4,.mov,.avi,.wmv,.webm,.mp3,.wav,.ogg,.m4a,.pdf,.doc,.docx,.xls,.xlsx,.txt,.zip,.rar" class="hidden">
                    </label>
                </div>

                <div wire:loading wire:target="answer_new_images" class="text-sm text-blue-600 mb-2 font-medium flex items-center gap-2">
