            ...
            <table class="table">
                ...
                <tbody>
                    @forelse ($transactionProducts as $transactionProduct)
                        <tr>
                            <td class="center">{{ $loop->iteration + ($transactionProducts->currentPage() - 1) * $transactionProducts->perPage() }}</td>
                            <td>
                                <p class="font-medium">{{ $transactionProduct->product->name }}</p>
                                <p class="text-xs text-gray-500">@Rp{{ number_format($transactionProduct->price, 0, ',', '.') }}</p>
                            </td>
        ...
        <div class="px-5 py-4 bg-gray-50/80 border-t border-gray-200">
            <div class="flex items-center justify-between">
                <div class="text-sm text-gray-700">
                    Menampilkan <span class="font-medium">{{ $transactionProducts->firstItem() }}</span> sampai <span class="font-medium">{{ $transactionProducts->lastItem() }}</span> dari <span class="font-medium">{{ $transactionProducts->total() }}</span> hasil
                </div>
                <div>
