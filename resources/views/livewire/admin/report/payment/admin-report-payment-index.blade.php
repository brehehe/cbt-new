<div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-4">
        ...
        @foreach ($payments as $payment)
            <div
                class="bg-white rounded-xl shadow-lg hover:shadow-xl transition-shadow duration-300 p-6 border border-gray-200">
                ...
                <p class="text-2xl font-bold text-green-600">Rp {{ number_format($payment->total_amount, 0, ',', '.') }}
                </p>
            </div>
        @endforeach
    </div>
        ...
        <div class="px-5 py-4 bg-gray-50/80 border-t border-gray-200">
            <div class="flex items-center justify-between">
                <div class="text-sm text-gray-700">
                    Menampilkan <span class="font-medium">{{ $transactionPayments->firstItem() }}</span> sampai <span
                        class="font-medium">{{ $transactionPayments->lastItem() }}</span> dari <span
                        class="font-medium">{{ $transactionPayments->total() }}</span> hasil
                </div>
                <div>
