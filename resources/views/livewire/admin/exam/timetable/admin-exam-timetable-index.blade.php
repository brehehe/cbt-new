                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    @forelse ($timetables as $index => $timetable)
                        <tr class="hover:bg-gray-50 transition-colors duration-200">
                            <td class="px-6 py-4 whitespace-nowrap text-center text-sm text-gray-500">{{ $timetables->firstItem() + $index }}</td>
                            <td class="px-6 py-4 whitespace-nowrap font-medium text-gray-900">{{ $timetable->name ?? '-' }}</td>
                            <td class="px-6 py-4 whitespace-nowrap">
                                <span class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full bg-blue-100 text-blue-800">
                                    {{ $timetable->timetableModule->questionType->name ?? '-' }}
                                </span>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{{ $timetable->timetableModule->name ?? '-' }}</td>
                            ...
                            <td class="px-6 py-4 text-sm text-gray-500 line-clamp-2 max-w-xs" title="{{ $timetable->timetableModule->description ?? '-' }}">
