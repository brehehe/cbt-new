                ...
                <tbody>
                    @forelse ($cashFlows as $cashFlow)
                        ...
                        <tr class="cursor-pointer hover:bg-gray-100 transition-colors category-row"
                            data-category-index="{{ $cashFlow }}"
                            onclick="TableController.toggleCategory('{{ $cashFlow }}')">
                            <td class="center">{{ $loop->iteration }}</td>
                            <td>
                                <div class="flex items-center gap-2">
                                    <svg class="w-4 h-4 transition-transform duration-200 category-arrow"
                                        data-category-arrow="{{ $cashFlow }}" fill="none" stroke="currentColor"
                                        viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                            d="M9 5l7 7-7 7"></path>
                                    </svg>
                                    <span class="font-medium">{{ $cashFlow ?? '-' }}</span>
                                </div>
                            </td>
                            <td>Rp {{ number_format($cashFlowTotals[$cashFlow]['total_debit'] ?? 0, 0, ',', '.') }}
                            </td>
                            <td>Rp {{ number_format($cashFlowTotals[$cashFlow]['total_credit'] ?? 0, 0, ',', '.') }}
                            </td>
                            <td>Rp {{ number_format($cashFlowTotals[$cashFlow]['total'] ?? 0, 0, ',', '.') }}</td>
                            <td>
                                <button
                                    onclick="event.stopPropagation(); TableController.toggleCategory('{{ $cashFlow }}')"
                                    class="text-blue-500 hover:text-blue-700 font-medium transition-colors">
                                    <span class="category-btn-text" data-category-btn="{{ $cashFlow }}">Buka</span>
                                </button>
                            </td>
                        </tr>

                        {{-- Level 2: Category Accounts --}}
                        @foreach ($categoryAccounts[$cashFlow] as $key => $categoryAccount)
                            <tr class="bg-gray-50 cursor-pointer hover:bg-gray-100 transition-colors subcategory-row"
                                data-parent-category="{{ $cashFlow }}"
                                data-subcategory-key="{{ $cashFlow }}_{{ $key }}" style="display: none;"
                                onclick="TableController.toggleSubCategory('{{ $cashFlow }}', '{{ $key }}')">
                                <td class="center pl-6">{{ $loop->iteration }}</td>
                                <td>
                                    <div class="flex items-center gap-2 pl-4">
                                        <svg class="w-4 h-4 transition-transform duration-200 subcategory-arrow"
                                            data-subcategory-arrow="{{ $cashFlow }}_{{ $key }}"
                                ...
                                <td>Rp
                                    {{ number_format($categoryAccountTotals[$cashFlow][$key]['total_debit'] ?? 0, 0, ',', '.') }}
                                </td>
                                <td>Rp
                                    {{ number_format($categoryAccountTotals[$cashFlow][$key]['total_credit'] ?? 0, 0, ',', '.') }}
                                </td>
                                <td>Rp
                                    {{ number_format($categoryAccountTotals[$cashFlow][$key]['total'] ?? 0, 0, ',', '.') }}
                                </td>
                                <td>
                                    <button
                                        onclick="event.stopPropagation(); TableController.toggleSubCategory('{{ $cashFlow }}', '{{ $key }}')"
                                        class="text-blue-500 hover:text-blue-700 font-medium transition-colors text-sm">
                                        <span class="subcategory-btn-text"
                                            data-subcategory-btn="{{ $cashFlow }}_{{ $key }}">Buka</span>
                                    </button>
                                </td>
                            </tr>

                            {{-- Level 3: Individual Accounts --}}
                            @foreach ($accounts[$cashFlow][$key] as $key_account => $account)
                                <tr class="bg-blue-50/50 hover:bg-blue-100/50 transition-colors account-row"
                                    data-parent-subcategory="{{ $cashFlow }}_{{ $key }}"
                                    style="display: none;">
                                    ...
                                    <td>Rp
                                        {{ number_format($accountTotals[$cashFlow][$key][$key_account]['total_debit'] ?? 0, 0, ',', '.') }}
                                    </td>
                                    <td>Rp
                                        {{ number_format($accountTotals[$cashFlow][$key][$key_account]['total_credit'] ?? 0, 0, ',', '.') }}
                                    </td>
                                    <td>Rp
                                        {{ number_format($accountTotals[$cashFlow][$key][$key_account]['total'] ?? 0, 0, ',', '.') }}
                                    </td>
                                    <td>
