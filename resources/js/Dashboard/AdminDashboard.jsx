        ...
        <div className="w-full font-sans selection:bg-[var(--primary)] selection:text-white">
                                ...
                                {statsData?.examStatistics ? (
                                    Object.entries(statsData.examStatistics).map(([status, count]) => {
                                        ...
                                        return (
                                            <a
                                                key={status}
                                                href={`/admin/exam/monitor?utStatus=${status}`}
                                                className={`p-4 rounded-2xl flex flex-col gap-2.5 ${styles.pill} transition-all hover:scale-[1.01] hover:opacity-90 cursor-pointer`}
                                            >
                                                <div className="flex items-center justify-between">
                            ...
                            <div>
                                ...
                                <div className="grid grid-cols-2 gap-4 mb-5">
                                    <div
                                        ...
                                        className="p-4 bg-sky-50/50 border border-sky-100 rounded-2xl text-center cursor-pointer hover:bg-sky-100/60 hover:scale-[1.02] transition-all"
                                        title="Cek Sesi Aktif"
                                    >
                                        <div className="text-3xl font-black text-sky-700">
                                            {realtimeData?.liveSessionStats?.active_sessions || 0}
                                        </div>
                                        <div className="text-[10px] font-bold text-sky-600 uppercase tracking-wider mt-1 flex items-center justify-center gap-1">
                                            <span>Sesi Kamera</span>
            ...
            {showActiveSessionsModal && (
                <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
                    {/* Backdrop */}
                    <div className="absolute inset-0 bg-slate-900/60 backdrop-blur-sm" onClick={() => setShowActiveSessionsModal(false)} />

                    {/* Panel */}
                    <div className="relative z-10 bg-white rounded-2xl shadow-2xl w-full max-w-2xl overflow-hidden flex flex-col max-h-[85vh] animate-in fade-in zoom-in-95 duration-250">
                        {/* Header */}
                        <div className="bg-blue-600 px-6 py-4 flex items-center justify-between text-white">
                            <div className="flex items-center gap-2">
                                ...
                                <h3 className="font-bold text-base uppercase tracking-wider">Sesi Ujian Aktif Saat Ini</h3>
                            </div>
                            <button onClick={() => setShowActiveSessionsModal(false)} className="text-white/80 hover:text-white transition-colors">
                        ...
                        <div className="p-6 overflow-y-auto flex-1 text-slate-600">
                            {!realtimeData?.activeSessionsList || realtimeData.activeSessionsList.length === 0 ? (
                                <div className="text-center py-12 text-slate-500">
                                    ...
                                    <p className="text-sm font-bold text-slate-700">Tidak Ada Sesi Aktif</p>
                                    <p className="text-xs mt-1 text-slate-400">Saat ini tidak ada mahasiswa yang sedang mengerjakan ujian.</p>
                                </div>
                            ) : (
                                <div className="overflow-x-auto">
                                    <table className="w-full text-sm text-left text-slate-500">
                                        <thead className="text-xs text-slate-700 uppercase bg-slate-50">
                                            <tr>
                                                <th scope="col" className="px-4 py-3">Mahasiswa</th>
                                                <th scope="col" className="px-4 py-3">Ujian / Modul</th>
                                            ...
                                            {realtimeData.activeSessionsList.map((session) => (
                                                <tr key={session.id} className="hover:bg-slate-50/50">
                                                    <td className="px-4 py-3 font-medium text-slate-900">
                                                        ...
                                                        <div className="text-xs text-slate-400 font-normal">{session.user?.nim || 'N/A'}</div>
                                                    </td>
                                                    <td className="px-4 py-3">
                                                        <div className="text-slate-700 font-medium">{session.timetable?.name || 'N/A'}</div>
                                                        <div className="text-xs text-slate-400">{session.timetable?.module_name || 'N/A'}</div>
                                                    </td>
                                                    <td className="px-4 py-3">
                                                        <div className="flex items-center gap-2">
                                                            <span className="font-semibold text-slate-700">{session.stats?.answered}/{session.stats?.total}</span>
                                                            <span className="text-xs text-slate-400">({session.stats?.percentage}%)</span>
                                                        </div>
                                                        <div className="w-24 bg-slate-200 rounded-full h-1.5 mt-1">
                                                            <div className="bg-blue-600 h-1.5 rounded-full" style={{ width: `${session.stats?.percentage}%` }} />
                                                        </div>
                                                    </td>
                                                    <td className="px-4 py-3 text-right whitespace-nowrap">
                                                        <a href={`/admin/exam/monitor?search=${session.user?.nim || session.user?.name}`} className="inline-flex items-center px-3 py-1.5 bg-blue-50 text-blue-600 hover:bg-blue-100 font-bold rounded-lg text-xs transition-colors">
                                                            Pantau
                        ...
                        <div className="px-6 py-4 bg-slate-50 border-t border-slate-100 flex justify-end">
                            <button onClick={() => setShowActiveSessionsModal(false)} className="px-4 py-2 text-sm font-medium text-slate-700 bg-white border border-slate-300 rounded-lg hover:bg-slate-50 transition-colors">
                                Tutup
                            </button>
                        </div>
                    </div>
