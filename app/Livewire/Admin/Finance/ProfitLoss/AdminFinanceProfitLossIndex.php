...
class AdminFinanceProfitLossIndex extends Component
{
    ...
    public function balanceSheet()
    {
        ...
        $detailCategories = DetailCategoryAccount::with([
            ...
            ->where('type', 'profit-loss') // Hanya ambil kategori profit-loss
            ->orderByRaw("CASE
        WHEN LOWER(name) ILIKE '%Pendapatan%' THEN 1
        WHEN LOWER(name) ILIKE '%Beban Pokok%' THEN 2
        WHEN LOWER(name) ILIKE '%Beban%' THEN 3
        WHEN LOWER(name) ILIKE '%Pendapatan Lain%' THEN 4
        WHEN LOWER(name) ILIKE '%Beban Lain%' THEN 5
        ELSE 6
    ...
    {
        return view('livewire.admin.finance.profit-loss.admin-finance-profit-loss-index')
            ->extends('layout.app')
            ->section('content');
    }
}
